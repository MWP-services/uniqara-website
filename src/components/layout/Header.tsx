    ...
    <nav aria-label="Hoofdnavigatie" className={className}>
      {navigation.main.map((item) => (
        <Link
          key={item.href}
          href={item.href}
          className="rounded-pill px-3 py-2 text-sm font-semibold text-foreground transition hover:bg-surface hover:text-brand-green"
        >
          {item.label}
        </Link>
      ))}
      ...
      <Container>
        <div className="flex min-h-24 items-center justify-between gap-4 py-3">
          ...
          <details className="group relative lg:hidden">
            <summary className="list-none rounded-pill border border-border-soft bg-white px-4 py-3 text-sm font-semibold text-brand-green shadow-card transition hover:bg-surface">
              Menu
            </summary>
            <div className="absolute right-0 mt-3 w-[min(82vw,22rem)] rounded-medium border border-border-soft bg-white p-4 shadow-soft">
