...
- Kleuren: CSS-variabelen in `:root`, zoals `--background`, `--foreground`, `--brand-green`, `--accent-yellow`, `--surface` en `--border-soft`.
...
- Oppervlakken: `.surface-card` en `.surface-panel` leggen rustige kaders, zachte hoeken en subtiele schaduw vast.
- Focus: `--focus-ring` bepaalt de toegankelijke focusstijl voor links en knoppen.

Kleurgevoel: fris wit, bijna-zwarte tekst, natuurlijk groen, zacht warm geel en gedempte neutrale tinten. Er is bewust geen oranje gebruikt.
